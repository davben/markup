#' Estimate a gross output production function (as in DLW/DLS)
#'
#' @description
...
#' However, De Loecker and Scott (2016) (DLS) suggest, by using a serially correlated input price such as wages
...
#' For a Cobb-Douglas type, this requires three initial values, `c(bl = 0.4, bk = 0.3, bm = 0.3)`.
#' It is best practice to provide a named vector to avoid unnecessary, and potentially hard to debug mistakes.
#' Without any names, the estimation routine assumes the order of initial values to be (1) labour,
#' (2) capital, and (3) materials coefficient.
#'
#' ## Instruments
#' The moment evaluator function by default uses lagged materials as instruments as well as labor and capital.
...
#' For example, adding lagged labour as an instrument is done using `instruments = c("l", "l_lag", "k", "m_lag")`
#' in the `dlw_go()`-function.
#'
#' ## Data
...
dlw_go <- function(initial_values, data, type = c("cd", "tl"), ...) {
  type <- match.arg(type)

  if (type == "cd") {
    if (length(initial_values) != 3) stop("Estimating a gross output Cobb-Douglas function requires initial values for bl, bk, and bm.")
    initial_values <- check_initial_values(x = initial_values, type = "cdgo", version = "dlw")
    result <- stats::optim(par = initial_values, fn = dlw_go_gmm_cd, data = data, ...)
  }

  if (type == "tl") {
    if (length(initial_values) != 10) stop("Estimating a Translog function requires initial values for bl, bk, bm, bll, bkk, bmm, blk, blm, bkm, and blkm.")
    initial_values <- check_initial_values(x = initial_values, type = "tlgo", version = "dlw")
    result <- stats::optim(par = initial_values, fn = dlw_go_gmm_tl, data = data, ...)
  }
