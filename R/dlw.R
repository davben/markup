#' Estimate a structural value-added production function (as in DLW)
#'
#' @description
#' Using the two-stage proxy function approach as described by De Loecker and
#' Warzynski (2012), based on Ackerberg, Caves, and Frazer (2015),
#' estimate a Cobb-Douglas or translog structural value-added production function.
#' The sole difference to the `acf()`-function is the instrumentation in the second
#' stage GMM-estimation which also omits the intercept term.
#'
...
#' For a Cobb-Douglas type, this requires two initial values, `c(bl = 0.4, bk = 0.6)`.
...
#' For example, adding same-period labour as an instrument is done using `instruments = c("l", "l_lag", "k")`
#' in the `dlw()`-function.
#'
#' ## Data
...
dlw <- function(initial_values, data, type = c("cd", "tl"), ...) {
  type <- match.arg(type)

  if (type == "cd") {
    if (length(initial_values) != 2) stop("Estimating a Cobb-Douglas function requires initial values for bl, and bk.")
    initial_values <- check_initial_values(x = initial_values, type = "cd", version = "dlw")
    result <- stats::optim(par = initial_values, fn = dlw_gmm_cd, data = data, ...)
  }

  if (type == "tl") {
    if (length(initial_values) != 5) stop("Estimating a Translog function requires initial values for bl, bk, bll, bkk, and blk.")
    initial_values <- check_initial_values(x = initial_values, type = "tl", version = "dlw")
    result <- stats::optim(par = initial_values, fn = dlw_gmm_tl, data = data, ...)
  }
